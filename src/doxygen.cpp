	...
	)
	{
			...
			(root->groupDocType != Entry::GROUPDOC_NORMAL && additional))
		{
			...
			else
			{
				if (root->tagInfo())
				{
					gd = Doxygen::groupLinkedMap->add(root->name,
					                                  std::unique_ptr<GroupDef>(
						                                  createGroupDef(
							                                  root->fileName, root->startLine, root->name, root->type,
							                                  root->tagInfo()->fileName)));
				...
				gd->setDocumentation(!root->doc.isEmpty() ? root->doc : QCString(" "), root->docFile, root->docLine,
				                     FALSE);
				gd->setInbodyDocumentation(root->inbodyDocs, root->inbodyFile, root->inbodyLine);
				gd->addSectionsToDefinition(root->anchors);
				gd->setRefItems(root->sli);
...
static Definition* buildScopeFromQualifiedName(const QCString& name_, SrcLangExt lang, const TagInfo* tagInfo)
{
	...
	while (i < level)
	{
		...
		else if (nd == nullptr && cd == nullptr && fullScope.find('<') == -1)
...
static void addClassToContext(const Entry* root)
{
	...
	else // new class
	{
		...
		cd = toClassDefMutable(
			Doxygen::classLinkedMap->add(fullName,
			                             createClassDef(tagInfo ? tagName : root->fileName, root->startLine,
			                                            root->startColumn,
			                                            fullName, sec, tagName, refFileName,TRUE,
			                                            root->spec.isEnum())));
		if (cd)
		{
	...
	if (cd)
	{
		...
		if (!root->spec.isForwardDecl())
		{
			...
			{
				AUTO_TRACE_ADD("Inserting class {} in file {} (root->fileName='{}')", cd->name(), fd->name(),
				               root->fileName);
				cd->setFileDef(fd);
				fd->insertClass(cd);
			}
							...
							if (ncd)
							{
								...
								if (pml)
								{
									for (const auto& pmd : *pml)
									{
										...
										{
											pmdm->setAccessorType(
												ncd, substitute(pmd->typeString(), icd->name(), ncd->name()));
		...
		if (!fullName.isEmpty())
		{
			...
			if (ndi) // existing namespace
			{
				...
				if (nd) // non-inline namespace
				{
					...
					if (root->tagInfo() == nullptr && nd->isReference() && !(root->doc.isEmpty() && root->brief.
						isEmpty()))
			...
			else // fresh namespace
			{
				...
				NamespaceDefMutable* nd = toNamespaceDefMutable(
					Doxygen::namespaceLinkedMap->add(fullName,
					                                 createNamespaceDef(tagInfo ? tagName : root->fileName,
					                                                    root->startLine,
					                                                    root->startColumn, fullName, tagName,
					                                                    tagFileName,
...
static void findUsingDirectives(const Entry* root)
{
	if (root->section.isUsingDir())
	{
		...
		if (!name.isEmpty())
		{
			...
			{
				AUTO_TRACE_ADD("new unknown namespace {} lang={} hidden={}", name, langToString(root->lang),
				               root->hidden);
				// add namespace to the list
				nd = toNamespaceDefMutable(
					Doxygen::namespaceLinkedMap->add(name,
					                                 createNamespaceDef(root->fileName, root->startLine,
							...
							if (md && md->protection() != Protection::Private)
							{
								...
								if (!cd->containsOverload(md))
								{
									...
									if (!root->doc.isEmpty() || !root->brief.isEmpty())
									{
										...
										newMmd->setInbodyDocumentation(root->inbodyDocs, root->inbodyFile,
										                               root->inbodyLine);
									}
									else
									{
										newMmd->setDocumentation(md->documentation(), md->docFile(), md->docLine());
										newMmd->setBriefDescription(md->briefDescription(), md->briefFile(),
										                            md->briefLine());
										newMmd->setInbodyDocumentation(md->inbodyDocumentation(), md->inbodyFile(),
										                               md->inbodyLine());
									}
									...
									newMmd->setBodySegment(md->getDefLine(), md->getStartBodyLine(),
									                       md->getEndBodyLine());
	...
	Relationship related)
{
	...
	AUTO_TRACE(
		...
		root->fileName, type, qualScope, name, args, root->protection, mtype, lang, fromAnnScope,
		root->initializer.str());
...
static int findFunctionPtr(const std::string& type, SrcLangExt lang, int* pLength = nullptr)
{
	...
	if (be != std::string::npos)
	{
		...
		templFp = (cc_ast != std::string::npos && cc_ast > be) || (cc_amp != std::string::npos && cc_amp > be);
		// hack to find, e.g 'B<X>(A<int>::*)'
	}
	...
	)
	{
		...
		if (mn) // symbol with the same name already found
		{
			for (auto& imd : *mn)
			{
				...
				bool bothSameScope = (!cd && !nd) || (cd && imd->getClassDef() == cd) || (nd && imd->getNamespaceDef()
					== nd);
...
static void buildInterfaceAndServiceList(const Entry* root)
{
	if (root->section.isExportedInterface() || root->section.isIncludedService())
	{
		AUTO_TRACE("Exported interface/included service: type='{}' scope='{}' name='{}' args='{}'"
		           ...
		           root->relates, root->relatesType, root->fileName, root->startLine, root->bodyLine,
		           root->tArgLists.size(),
	...
	)
{
	...
	// adding class member
	Relationship relationship = relates.isEmpty()
		? Relationship::Member
		: root->relatesType == RelatesType::MemberOf
		? Relationship::Foreign
		: Relationship::Related;
	auto md = createMemberDef(
		fileName, root->startLine, root->startColumn,
					...
					for (const auto& imd : *mn)
					{
						...
						if (md)
						{
							...
							)
							{
								...
								AUTO_TRACE_ADD("combining function with prototype found={} in namespace '{}'", found,
								               nsName);
								...
								if (found)
								{
									...
									mergeArguments(const_cast<ArgumentList&>(mdAl), mergedArgList,
									               !root->doc.isEmpty());
									// merge documentation
			...
			for (const auto& ifmd : *fn)
			{
				...
				for (const auto& immd : *mn)
				{
					...
					// function is actually a friend.
					{
						...
						{
							mmd->setInbodyDocumentation(fmd->inbodyDocumentation(), fmd->inbodyFile(),
							                            fmd->inbodyLine());
						}
						else if (!mmd->inbodyDocumentation().isEmpty())
						{
							fmd->setInbodyDocumentation(mmd->inbodyDocumentation(), mmd->inbodyFile(),
							                            mmd->inbodyLine());
						}
		...
		for (auto& mi : *mni)
		{
			...
			if (md->isVariable() || md->isObjCProperty()) // for each member variable in this class
			{
				...
				while (!found && extractClassNameFromType(type, pos, usedClassName, templSpec, root->lang) != -1)
				{
					...
					findClassRelation(root, context, instanceCd, &bi, formTemplateNames, TemplateInstances,
					                  isArtificial);
	...
	)
{
	...
	// (in case of nested classes)
	{
		...
		do // try all parent scope prefixes, starting with the largest scope
		{
			...
			ClassDefMutable* baseClass = resolver.resolveClassMutable(
			...
			)
			{
				...
				if (found)
				{
					...
					else if (mode == DocumentedOnly || mode == Undocumented)
					{
						...
						if (cd != baseClass && !cd->isSubClass(baseClass) && baseClass->isBaseClass(cd, true, templSpec)
							== 0) // check for recursion, see bug690787
						{
				...
				else if (mode == Undocumented && (scopeOffset == 0 || isATemplateArgument))
				{
							...
							if (baseClass) // really added (not alias)
							{
								...
								if (si != -1) // class is nested
								{
									Definition* sd = findScopeFromQualifiedName(
										Doxygen::globalScope, baseClassName.left(si), nullptr, root->tagInfo());
					...
					if (baseClass)
					{
						...
						if (!cd->isSubClass(baseClass) && cd != baseClass && cd->isBaseClass(baseClass, true, templSpec)
...
static void computeClassRelations()
{
	...
	for (const auto& [name,root] : g_classEntries)
	{
		...
		if ((cd == nullptr || (!cd->hasDocumentation() && !cd->isReference())) && numMembers > 0 && !bName.
			endsWith("::"))
		{
				...
				if (namespaceName.left(enumNamePos) == nd->name())
				{
					...
					if (enumMn)
					{
						for (const auto& emd : *enumMn)
						{
							...
							if (found)
							{
								addMemberDocs(root, toMemberDefMutable(md->resolveAlias()), decl, nullptr,FALSE,
								              root->spec);
								break;
							}
						}
	...
	for (const auto& imd : *mn)
	{
		...
		if (cd && tcd == cd) // member's classes match
		{
			...
			AUTO_TRACE_ADD("matching '{}'<=>'{}' className='{}' namespaceName='{}'",
			               argListToString(argList,TRUE), argListToString(root->argList,TRUE), className,
			               namespaceName);
			...
			if (!md->templateArguments().empty() && !root->tArgLists.empty())
			{
				...
				AUTO_TRACE_ADD("Comparing return types '{}'<->'{}' #args {}<->{}",
				               md->typeString(), funcType, md->templateArguments().size(),
				               root->tArgLists.back().size());
	...
	)
{
	...
	if (!funcName.isEmpty()) // function name is valid
	{
		...
		if (!className.isEmpty() && (mn = Doxygen::functionNameLinkedMap->find(className)))
		{
			for (const auto& imd : *mn)
			{
				...
				if (md && md->isEnumerate() && md->isStrong() && (mdScope = md->getOuterScope()) &&
					...
					((namespaceName.isEmpty() && mdScope == Doxygen::globalScope) || (mdScope->name() ==
						namespaceName)))
				{
		...
		if (mn == nullptr) //unreal engine _implementation search
		{
			...
			for (const auto& objCMethod : objCImpl->children())
			{
				if (objCMethod->section.isFunction())
				{
					...
					findMember(objCMethod.get(),
					           ...
					           objCMethod->type,
					           objCMethod->args,
					           objCMethod->type + " " + objCImpl->name + "::" + objCMethod->name + " " + objCMethod->
					           args,
					           FALSE,TRUE);
...
static void findEnums(const Entry* root)
{
	if (root->section.isEnum())
	{
		...
		if (i != -1) // scope is specified
		{
			...
			{
				nd = toNamespaceDefMutable(
					buildScopeFromQualifiedName(root->name.left(i + 2), root->lang, root->tagInfo()));
			}
		}
		else // no scope, check the scope in which the docs where found
		{
			if (root->parent()->section.isScope() && !root->parent()->name.isEmpty())
				...
				for (const auto& imd : *mn)
				{
					...
					if (md && md->isEnumerate() && !root->children().empty())
					{
						...
						for (const auto& e : root->children())
						{
							...
							bool isJavaLike = sle == SrcLangExt::CSharp || sle == SrcLangExt::Java || sle ==
								SrcLangExt::XML;
							if (isJavaLike || root->spec.isStrong())
							{
								...
								if (md->qualifiedName() == qualifiedName) // enum value scope matches that of the enum
								{
									...
									fmmd->setExplicitExternal(e->explicitExternal, fileName, e->startLine,
									                          e->startColumn);
							...
							else
							{
								...
								// get list of members with the same name as the field
								{
									...
									{
										MemberDefMutable* fmd = toMemberDefMutable(ifmd.get());
										if (fmd && fmd->isEnumValue() && fmd->getOuterScope() == md->getOuterScope())
										// in same scope
										{
											...
											if (nd && !nd->isAnonymous())
											{
												if (!fmd->isStrongEnumValue())
												// only non strong enum values can be globally added
												{
...
static void findEnumDocumentation(const Entry* root)
{
	...
	)
	{
		...
		if (!name.isEmpty())
		{
			...
			{
				MemberName* mn = cd
					? Doxygen::memberNameLinkedMap->find(name)
					: Doxygen::functionNameLinkedMap->find(name);
				if (mn)
				{
					for (const auto& imd : *mn)
					{
...
static void generateFileSources()
{
	auto processSourceFile = [](FileDef* fd, OutputList& ol, ClangTUParser* parser)
	{
		...
		bool parseSources = !fd->isReference() && Doxygen::parseSourcesNeeded;
		...
		else
		{
			PageDef* pd = Doxygen::exampleLinkedMap->add(root->name,
			                                             createPageDef(root->fileName, root->startLine,
			                                                           root->name,
			                                                           root->brief + root->doc + root->inbodyDocs,
			                                                           root->args));
			pd->setBriefDescription(root->brief, root->briefFile, root->briefLine);
...
static void generateDiskNames()
{
	for (const auto& fn : *Doxygen::inputNameLinkedMap)
	{
		struct FileEntry
		{
			FileEntry(const QCString& p, FileDef* fd) :
				path(p), fileDef(fd) {}
...
bool writeFile(const char* fileName, std::function<void(TextStream&)> func)
{
	std::ofstream f;
...
void readConfigfile(const RunOptions& options)
{
  Config::CompareMode defaultDiffList = Config::CompareMode::Full;
	...
	if (options.diffList != Config::CompareMode::Full)
	{
		...
		compareDoxyfile(options.diffList);
		cleanUpDoxygen();
		exit(0);
	}
	*/
...
void createLayoutAndExit(const QCString& layoutName)
{
...
void parseMainArgs(const RunOptions& options)
{
	...
	initTracing(options.traceName.data(), options.traceTiming);
...
void searchInputFiles()
{
	...
	for (const auto& s : includePathList)
	{
		...
		const StringVector& pl =
			plSize == 0 ? Config_getList(FILE_PATTERNS) : Config_getList(INCLUDE_FILE_PATTERNS);
...
void generateOutput()
{
		...
		&& !Config_getBool(USE_MATHJAX))
	{
		...
		fm.generateImages(
			...
			: FormulaManager::Format::Bitmap, FormulaManager::HighDPI::On);
		g_s.end();
	}
	...
	int typeCacheParam = computeIdealCacheParam(static_cast<size_t>(Doxygen::typeLookupCache->misses() * 2 / 3));
