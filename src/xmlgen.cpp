...
static void stripAnonymousMarkers(QCString& s)
{
	...
	if (len > 0)
	{
		while (i < len)
		{
			...
			{
				if (j >= 2 && i >= 2 && s[i - 2] == ':' && s[i - 1] == ':')
					j -= 2; // found pattern '::@\d+'
				i += 2; // skip over @ and first digit
				while (i < len && isDigit(s[i]))
					i++; // skip additional digits
			}
...
static void generateXMLForMember(const MemberDef* md, TextStream& ti, TextStream& t, const Definition* def)
{
	...
	case MemberType::Enumeration:
		memType = "enum";
		break;
	case MemberType::EnumValue: ASSERT(0);
		break;
	...
	if (isFunc)
	{
		...
		{
			t << " refqual=\"";
			if (al.refQualifier() == RefQualifierType::LValue)
				t << "lvalue";
			else
				t << "rvalue";
			t << "\"";
		...
		{
		case Specifier::Normal:
			t << "non-virtual";
			break;
		case Specifier::Virtual:
			t << "virtual";
			break;
		case Specifier::Pure:
			t << "pure-virtual";
...
static void generateXMLForClass(const ClassDef* cd, TextStream& ti)
{
	...
	for (const auto& bcd : cd->baseClasses())
	{
		...
		case Protection::Private:
			t << "private";
			break;
		case Protection::Package: ASSERT(0);
			break;
		}
		t << "\" virt=\"";
		switch (bcd.virt)
...
{
	if (mod->isReference() || mod->isHidden() || !mod->isPrimaryInterface())
		return;
...
static void generateXMLForPage(PageDef* pd, TextStream& ti, bool isExample)
{
	...
	if (pd->localToc().isXmlEnabled() && !sectionRefs.empty())
	{
		...
		auto writeIndent = [&]()
		{
			for (int i = 0; i < 4 + indent * 2; i++)
				t << " ";
		};
		auto incIndent = [&](const char* text)
		{
			writeIndent();
			t << text << "\n";
			indent++;
		};
		auto decIndent = [&](const char* text)
		{
			indent--;
			writeIndent();
			t << text << "\n";
